 ...
 */
export function validateInstallationName(name) {
 ...
 */
export async function createInstallationFolder(installationName, basePath = process.cwd()) {
    // Validate installation name
    if (!validateInstallationName(installationName)) {
        throw new Error(`Invalid installation name: "${installationName}". ` +
    ...
    try {
        ...
        throw new Error(`Installation folder already exists: ${installationPath}`);
    }
    catch (error) {
 ...
 */
export async function mirrorOntologyStructure(installationPath) {
    const ontologyDirs = [
        "groups",
        "people",
        "things",
 ...
 */
export async function createReadme(installationPath, organizationName, installationIdentifier) {
    // Read template
    const templatePath = path.resolve(__dirname, "../../templates/installation-readme.md");
    let template;
    try {
 ...
 * @param basePath Optional base path (defaults to cwd)
 */
export async function updateEnvFile(installationName, basePath = process.cwd()) {
    ...
    if (installationNamePattern.test(envContent)) {
        ...
        envContent = envContent.replace(installationNamePattern, `INSTALLATION_NAME=${installationName}`);
    }
    else {
        // Append new value
 ...
 */
export async function updateOrgEnvFile(options, basePath = process.cwd()) {
 ...
 * @param basePath Optional base path (defaults to cwd)
 */
export async function updateGitignore(installationName, exclude, basePath = process.cwd()) {
    if (!exclude) {
        return;
    }
 ...
 * @param installationPath Path to installation folder
 */
export async function rollbackInstallation(installationPath) {
    try {
 ...
 */
export async function createOnboardingFile(installationPath, data) {
    ...
    await fs.writeFile(onboardingPath, JSON.stringify(onboardingData, null, 2), "utf-8");
}
/**
 * Default README template (inline fallback)
 */
function getDefaultReadmeTemplate() {
...
//# sourceMappingURL=installation-setup.js.map