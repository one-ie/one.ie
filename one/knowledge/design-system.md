...
ONE Platform's design system is built on **6 extractable color tokens + 4 design properties** that create beautiful, consistent components across the entire platform. Every card, button, and component maps to these tokens.
...
**Foundation:** Built on shadcn/ui components with Tailwind CSS v4, extending the base system with ontology-aware design patterns.

---

## The 6 Color Tokens + 4 Design Properties
...
@theme {
  /* 6 Core Color Tokens */
  --color-background: 0 0% 93%;      /* Light gray card surface */
  --color-foreground: 0 0% 100%;     /* White content area */
  --color-font: 0 0% 13%;            /* Dark text */
  --color-primary: 216 55% 25%;      /* Blue CTA */
  --color-secondary: 219 14% 28%;    /* Gray-blue support */
  --color-tertiary: 105 22% 25%;     /* Green accent */
  ...
  /* Design Properties */
  /* States: defined in component CSS */
...
**Migration Strategy:**
1. CSS variables will support both old and new names during transition
2. Components updated to use new token names
3. Old tokens deprecated but functional for 1 version cycle
4. Full removal in version 2.0.0
...
- Use `primary` for main CTAs, `secondary` for supporting actions, `tertiary` for accents
- Keep `background`/`foreground` as neutral grays
- Use opacity (`opacity-80`, `opacity-60`) for text hierarchy, not different colors
- Apply the "frame + content" pattern consistently across all cards

### DON'T ❌

- **Don't override colors unless absolutely necessary**
- Don't use more than 6 base colors per thing
- Don't hardcode HSL values in components (use CSS variables)
- Don't override `font` color without testing readability (WCAG AA)
- Don't use `tertiary` for critical actions (it's for accents only)
- Don't mix color systems (stick to 6 tokens)
- **Don't assume every thing needs custom colors** (default is perfect!)
- Don't use custom shadows, radius, or motion values (use design properties)
...
- `@agent-designer` auto-checks contrast when extracting colors

**Font Color Rules:**
- Light mode: `font` (dark) on `foreground` (white) = 15.8:1 ✅
- Dark mode: `font` (white) on `foreground` (dark) = 15.3:1 ✅
- Default values guarantee WCAG AAA compliance

**Keyboard Navigation:**
- All interactive elements have visible focus states (`ring-2 ring-primary-dark`)
...
```typescript
// Global platform colors (web/src/styles/global.css)
{
...
<Card className="bg-background p-1">
  <CardContent className="bg-foreground p-4">
    <h3 className="text-font">Welcome to ONE Platform</h3>
...
```typescript
// Purple-themed product (thing.colors override)
{
...
**Usage:**
```tsx
<ThingCard thing={purpleProduct}>
  {/* Automatically applies purple brand colors */}
...
```typescript
// Orange-themed course (thing.colors override)
{
...
**Usage:**
```tsx
<ThingCard thing={orangeCourse}>
  {/* Automatically applies orange brand colors */}
  <Button variant="primary">Enroll Now</Button>
  <Button variant="tertiary">Limited Seats</Button>
</ThingCard>
```
...
**@agent-designer** is the specialized agent for design system implementation. It:

1. **Extracts colors from websites** using screenshots or URLs
2. **Validates WCAG compliance** for all color combinations
3. **Generates thing-level color overrides** when requested
4. **Creates design tokens** in the correct format
5. **Updates components** to use the new design system

**Usage:**
```
@agent-designer extract colors from https://stripe.com
@agent-designer validate contrast for purple theme
@agent-designer create thing colors for product X based on brand guidelines
```
...
Our design system is **built on top of shadcn/ui**, extending it with:

- **6-token color system** (instead of shadcn's 10+ tokens)
- **Thing-level overrides** (ontology-aware branding)
- **Simplified naming** (background, foreground, font instead of card, popover, muted)
- **Consistent component patterns** (frame + content cards)
- **Design properties** (states, elevation, radius, motion)

**shadcn Components Used:**
- 50+ UI components in `/web/src/components/ui/`
- Button, Card, Input, Select, Dialog, Dropdown, etc.
- All extended with 6-token system
...
- **CSS Variables:** `/web/src/styles/global.css`
...
- **Web CLAUDE.md:** `/web/CLAUDE.md` (frontend context)
...
- 6 core color tokens (background, foreground, font, primary, secondary, tertiary)
- 4 design properties (states, elevation, radius, motion)
...
- Migration guide from old shadcn tokens
