...
export class GroupService extends Context.Tag("GroupService")<
	GroupService,
	{
		...
		readonly getBySlug: (
			slug: string,
		) => Effect.Effect<Group, GroupNotFoundError>;
		readonly list: (
			options?: ListGroupsOptions,
		) => Effect.Effect<Group[], QueryError>;
		readonly create: (
			input: CreateGroupInput,
		) => Effect.Effect<string, GroupCreateError>;
		readonly update: (
			id: string,
...
>() {
	...
	static get(
		id: string,
	): Effect.Effect<Group, GroupNotFoundError, GroupService> {
	...
	static create(
		input: CreateGroupInput,
	): Effect.Effect<string, GroupCreateError, GroupService> {
	...
	static delete(
		id: string,
	): Effect.Effect<void, GroupNotFoundError, GroupService> {
	...
	static countByType(
		type: GroupType,
	): Effect.Effect<number, QueryError, GroupService> {
		return Effect.map(
			GroupService.filterByType(type),
	...
	static findByName(
		name: string,
	): Effect.Effect<Group | null, QueryError, GroupService> {
		return Effect.map(
			GroupService.list(),
			(groups) =>
