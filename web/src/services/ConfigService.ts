 ...
 */
export class ConfigUnauthorizedError extends Data.TaggedError(
	"ConfigUnauthorizedError",
)<{
	userId: string;
	organizationId: string;
	requiredRole: string;
 ...
 */
export class ConnectionTestError extends Data.TaggedError(
	"ConnectionTestError",
)<{
	providerType: string;
	reason: string;
...
export function switchProviderForOrganization(
	...
	return Effect.gen(function* () {
		const configService = yield* ConfigService;
		return yield* configService.switchProvider(
			organizationId,
			newConfig,
			actorId,
		);
