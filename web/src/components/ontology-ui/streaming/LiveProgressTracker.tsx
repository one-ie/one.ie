...
export function LiveProgressTracker({
  ...
  const overallProgress =
    totalProgress || steps.reduce((sum, step) => sum + (step.progress || 0), 0) / totalSteps;
      ...
      <div className="flex-1 min-w-0">
        ...
        {(step.startedAt || step.completedAt) && (
          <div className="flex items-center gap-4 text-xs text-muted-foreground mt-2">
            {step.startedAt && <span>Started {formatRelativeTime(step.startedAt)}</span>}
            {step.completedAt && <span>Completed {formatRelativeTime(step.completedAt)}</span>}
          </div>
        )}
      </div>
    </motion.div>
