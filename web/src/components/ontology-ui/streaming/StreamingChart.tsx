...
export function StreamingChart({
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    ...
    return (
      <div className="bg-popover border border-border rounded-lg shadow-lg p-3">
        <p className="text-xs text-muted-foreground mb-1">{formatXAxis(dataPoint[xAxisKey])}</p>
        <p className="text-sm font-semibold">{formatNumber(dataPoint[yAxisKey])}</p>
  ...
  const renderChart = () => {
    ...
    const axisProps = {
      ...
      grid: showGrid ? <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" /> : null,
