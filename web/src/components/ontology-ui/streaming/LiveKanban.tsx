...
export function LiveKanban({
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const activeCard = columns.flatMap((col) => col.cards).find((card) => card.id === active.id);
    ...
    if (targetColumn) {
      ...
      const targetCard = columns.flatMap((col) => col.cards).find((card) => card.id === over.id);
      if (targetCard) {
...
function KanbanColumnComponent({
  ...
  return (
    <div ref={setNodeRef} className="flex flex-col w-80 shrink-0 bg-muted/30 rounded-lg">
      {/* Column Header */}
          ...
          <div className="flex items-center justify-between">
            ...
            {card.assignee && (
              <Avatar className="h-6 w-6">
                ...
                <AvatarFallback className="text-xs">{card.assignee.name[0]}</AvatarFallback>
              </Avatar>
            )}
          </div>
        </CardContent>
