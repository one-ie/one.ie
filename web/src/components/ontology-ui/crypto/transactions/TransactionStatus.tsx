...
export function TransactionStatus({
  ...
  const progress = Math.min((transaction.confirmations / targetConfirmations) * 100, 100);
    ...
    const interval = setInterval(() => {
      ...
      setTransaction((prev) => {
        ...
        const updated = {
          ...
          status: newConfirmations >= targetConfirmations ? ("confirmed" as const) : prev.status,
        };
      ...
      <CardContent className="space-y-4">
        ...
        {timeRemaining !== null && !isComplete && !isFailed && (
          <div className="text-center p-4 bg-muted rounded-lg">
            <div className="text-sm text-muted-foreground mb-1">Estimated Time Remaining</div>
        ...
        {isFailed && (
          <div className="text-center p-4 bg-destructive/10 border border-destructive/20 rounded-lg">
            <div className="text-lg font-medium text-destructive">✗ Transaction Failed</div>
            {transaction.error && (
              <div className="text-sm text-muted-foreground mt-1">{transaction.error}</div>
            )}
          </div>
        )}
        ...
        <div className="space-y-2 text-sm">
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono">{transaction.blockNumber || "Pending..."}</span>
          </div>
          <div className="flex items-center justify-between">
