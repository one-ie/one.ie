...
import type { BorrowPosition, CollateralPosition } from "@/lib/services/crypto/LendingService";
...
export function CollateralManager({
  ...
  const totalBorrowedValue = borrowPositions.reduce((sum, p) => sum + parseFloat(p.amount), 0);
    ...
    <div className="space-y-6">
        ...
        <CardContent className="space-y-6">
          <div className="flex items-center justify-center">
            <div className="text-center">
              ...
              <Badge variant={getRiskVariant(healthFactor)} className="mt-4">
          ...
          {healthFactor < 2.0 && (
            <Alert variant={healthFactor < 1.5 ? "destructive" : "default"}>
              <AlertDescription>
                {healthFactor < 1.5 ? (
                  <>
                    <strong>⚠️ High Risk:</strong> Your position is at risk of liquidation. Add
                    collateral or repay borrowed funds immediately.
                  </>
                ) : (
                  <>
                    <strong>⚠️ Moderate Risk:</strong> Your health factor is below 2.0. Consider
                    adding more collateral to reduce liquidation risk.
                  </>
                )}
              </AlertDescription>
            </Alert>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {collateral.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">No collateral deposited</div>
          ) : (
