...
export function LendingMarketComponent({
  ...
  useEffect(() => {
    ...
    // Search filter
    if (searchQuery) {
      filtered = filtered.filter((m) => m.asset.toLowerCase().includes(searchQuery.toLowerCase()));
    }
  ...
  const avgSupplyAPY =
    markets.length > 0 ? markets.reduce((sum, m) => sum + m.supplyAPY, 0) / markets.length : 0;
  const avgBorrowAPY =
    markets.length > 0 ? markets.reduce((sum, m) => sum + m.borrowAPY, 0) / markets.length : 0;
                ...
                <Card key={index} className="hover:shadow-md transition-shadow">
                  <CardContent className="p-4">
                    <div className="grid grid-cols-7 gap-4 items-center">
                      ...
                      {/* Action */}
                      <div className="text-right">
                        <Button size="sm" onClick={() => onSelectMarket?.(market)}>
                          View
                        </Button>
                      </div>
                    </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                Supply your tokens to earn interest. You'll receive receipt tokens (aTokens for
                Aave, cTokens for Compound) that accrue interest over time.
              </p>
            </div>
            <div className="space-y-2">
              <h3 className="font-semibold">Borrow</h3>
              <p className="text-sm text-muted-foreground">
                Borrow tokens by supplying collateral. Your collateral must exceed your borrowed
                amount by the collateral factor to avoid liquidation.
              </p>
            </div>
            <div className="space-y-2">
              <h3 className="font-semibold">Utilization</h3>
              <p className="text-sm text-muted-foreground">
                Shows how much of the available supply is currently borrowed. Higher utilization
                typically means higher APYs.
              </p>
            </div>
            <div className="space-y-2">
              <h3 className="font-semibold">Collateral Factor</h3>
              <p className="text-sm text-muted-foreground">
                The percentage of your collateral value you can borrow against. For example, 0.75
                means you can borrow up to 75% of your collateral value.
              </p>
            </div>
          </div>
        </CardContent>
