...
export function LiquidationWarning({
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const drop = currentPrice > 0 ? ((currentPrice - liquidationPx) / currentPrice) * 100 : 0;
  ...
  const totalCollateralValue = position.collateral.reduce((sum, c) => sum + parseFloat(c.value), 0);
    ...
    <div className="space-y-6">
      ...
      {alertShown && riskLevel !== "safe" && (
        <Alert variant={riskLevel === "danger" ? "destructive" : "default"}>
          <AlertTitle>{riskLevel === "danger" ? "🚨 Liquidation Risk!" : "⚠️ Warning"}</AlertTitle>
          <AlertDescription>
            {riskLevel === "danger"
      ...
      <Card className={getRiskBg(riskLevel)}>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            {position.collateral.map((c, index) => (
              <div
                key={index}
                className="flex justify-between items-center text-sm bg-background/50 p-3 rounded"
              >
        ...
        {riskLevel !== "safe" && (
          <CardFooter className="flex gap-2">
            <Button variant="default" className="flex-1" onClick={onAddCollateral}>
              Add Collateral
            </Button>
            <Button variant="outline" className="flex-1" onClick={onRepayLoan}>
              Repay Loan
            </Button>
          </CardFooter>
        )}
