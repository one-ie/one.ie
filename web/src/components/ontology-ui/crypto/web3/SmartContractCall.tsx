...
export function SmartContractCall({ defaultAddress = "", onCallComplete }: SmartContractCallProps) {
  // Contract details
  ...
  const handleEstimateGas = async () => {
    ...
    try {
      ...
      await Effect.runPromise(validateFunctionArgs(selectedFunction, args));
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            {isValidAddress && (
              <div className="text-sm text-green-600 mt-2">✓ Valid contract address</div>
            )}
          </div>
          ...
          <div>
            ...
            />
            <Button onClick={handleParseABI} disabled={!abiInput} className="mt-2" size="sm">
              Parse ABI
            </Button>
          </div>
          ...
          {parsedABI && (
            <div className="text-sm text-muted-foreground">
              Found {parsedABI.functions.length} functions, {parsedABI.events.length} events
            </div>
          )}
        </CardContent>
      </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <Select value={selectedFunction?.name} onValueChange={handleSelectFunction}>
            ...
            {selectedFunction && (
              <>
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold">{selectedFunction.name}</span>
                    <Badge className={getStateMutabilityColor(selectedFunction.stateMutability)}>
                      {selectedFunction.stateMutability}
                    </Badge>
                    {isReadFunction && <Badge variant="outline">READ ONLY</Badge>}
                  </div>
                  ...
                  {selectedFunction.inputs.length > 0 ? (
                    ...
                    <div className="text-sm text-muted-foreground">No arguments required</div>
                  )}
                </div>
              </>
            )}
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
                ...
                {gasEstimate && (
                  <div className="p-3 bg-muted rounded-lg space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold">{gasEstimate.gasLimit.toString()}</span>
                    </div>
                    <div className="flex justify-between">
            ...
            <Button
              ...
              size="lg"
            >
              {loading ? "Executing..." : isReadFunction ? "Read Contract" : "Send Transaction"}
            </Button>
                ...
                <div className="p-4 bg-muted rounded-lg space-y-3">
                  ...
                  <div className="space-y-2 text-sm">
                    {callResult.success && (
                      <>
                        <div>
                          <div className="text-muted-foreground">Return Data:</div>
                        ...
                        {callResult.gasUsed && (
                          <div>
                            ...
                            <div className="font-semibold">{callResult.gasUsed.toString()}</div>
                          </div>
                        )}
                      </>
                    )}
