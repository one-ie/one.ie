...
export function MultiSigWallet({
  ...
  const handleCreateWallet = async () => {
    ...
    try {
      ...
      const newWallet = await Effect.runPromise(createMultiSig(validOwners, threshold));
  ...
  const handleSignTransaction = async (transaction: MultiSigTransaction) => {
    ...
    try {
      const signedTx = await Effect.runPromise(signTransaction(transaction, userAddress, wallet));

      setTransactions(transactions.map((tx) => (tx.id === transaction.id ? signedTx : tx)));
    } catch (err) {
      setError((err as Error).message);
    } finally {
      setLoading(false);
  ...
  const handleExecuteTransaction = async (transaction: MultiSigTransaction) => {
    ...
    try {
      const result = await Effect.runPromise(executeTransaction(transaction, wallet));

      if (result.success) {
        setTransactions(
          transactions.map((tx) => (tx.id === transaction.id ? { ...tx, executed: true } : tx))
        );
  ...
  const canExecute = (transaction: MultiSigTransaction) => {
    ...
    return transaction.signatures.length >= wallet.threshold && !transaction.executed;
  };
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <Button onClick={handleAddOwner} variant="outline" size="sm" className="mt-2">
                + Add Owner
              </Button>
            </div>
      ...
      {wallet && (
        <>
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="p-4 bg-muted rounded-lg space-y-3">
                ...
                <div>
                  <div className="text-sm text-muted-foreground mb-2">Signers</div>
                  <div className="space-y-1">
                    {wallet.owners.map((owner, index) => (
                      <div key={index} className="flex items-center justify-between text-sm">
                        <span className="font-mono">{owner}</span>
                        {owner === userAddress && <Badge variant="outline">You</Badge>}
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle>
                Transactions ({transactions.filter((tx) => !tx.executed).length} pending)
              </CardTitle>
            </CardHeader>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          {transaction.executed ? (
                            ...
                            <Badge variant="outline">
                              {transaction.signatures.length}/{wallet.threshold} signatures
                            </Badge>
                          )}
                        </div>
                        ...
                        {!transaction.executed && (
                          <>
                            <div>
                              ...
                              <Progress value={getSignatureProgress(transaction)} />
                            </div>
