...
import { AlertCircle, CheckCircle2, Shield, TrendingDown, Wallet, Zap } from "lucide-react";
...
export function StablecoinPay({
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Pay with Stablecoins</CardTitle>
        <CardDescription>No slippage, instant settlement, lowest fees</CardDescription>
      </CardHeader>
      ...
      <CardFooter>
        <Button
          ...
          onClick={() => {
            if (selectedOption) {
              onPaymentSelect?.(selectedOption.symbol, selectedOption.chain, priceUSD);
            }
          }}
        >
          {selectedOption ? (
            <>
              ...
              Pay ${priceUSD.toFixed(2)} with {selectedOption.symbol} on {selectedOption.chain}
            </>
          ) : (
