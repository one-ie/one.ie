...
import { formatPriceChange, getPriceChangeColor } from "@/lib/services/CryptoService";
...
export function TokenPrice({
  ...
  const sparklineColor = price.usd_24h_change >= 0 ? "rgb(34, 197, 94)" : "rgb(239, 68, 68)";
      ...
      <CardContent>
        ...
        <div className="mb-6">
          <div className="flex items-baseline gap-3 mb-2">
            <span className="text-4xl font-bold">${price.usd.toLocaleString()}</span>
        ...
        {showMultipleCurrencies && price.eth && price.btc && (
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div>
              ...
              <p className="font-mono font-medium">{price.eth.toFixed(6)} ETH</p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground mb-1">BTC Price</p>
              <p className="font-mono font-medium">{price.btc.toFixed(8)} BTC</p>
            </div>
          </div>
        )}
