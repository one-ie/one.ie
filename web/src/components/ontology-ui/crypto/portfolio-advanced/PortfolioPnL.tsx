...
export function PortfolioPnL({ tokens, className }: PortfolioPnLProps) {
  ...
  const topLosers = sortedByGains
    .filter((t) => t.totalGains < 0)
    .slice(-3)
    .reverse();
      ...
      <CardContent className="space-y-6">
        ...
        <Tabs defaultValue="all" className="w-full">
          <TabsList className="grid w-full grid-cols-3">
            ...
            <TabsTrigger value="gainers">Top Gainers ({topGainers.length})</TabsTrigger>
            <TabsTrigger value="losers">Top Losers ({topLosers.length})</TabsTrigger>
          </TabsList>
