...
export function YieldAggregator({ walletBalance = 50000, onDeposit }: YieldAggregatorProps) {
  // Search parameters
  ...
  const [allocation, setAllocation] = useState<Array<YieldOpportunity & { allocation: number }>>(
    []
  );
  ...
  const fetchOpportunities = async () => {
    ...
    try {
      ...
      const results = await Effect.runPromise(aggregateYields(amt, riskTolerance));
  ...
  const totalEarnings = positions.reduce((sum, pos) => sum + pos.estimatedEarnings, 0);
  const weightedAPY =
    totalDeposited > 0
      ? positions.reduce((sum, pos) => sum + (pos.apy * pos.amount) / totalDeposited, 0)
      : 0;
  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <Label>Sort By</Label>
              <Select value={sortBy} onValueChange={(v) => setSortBy(v as "apy" | "risk" | "tvl")}>
                <SelectTrigger className="mt-2">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          <Button onClick={fetchOpportunities} disabled={loading} className="w-full">
            ...
            {allocation.map((opp, index) => (
              <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
                ...
                <div className="text-right">
                  <div className="font-bold">${opp.allocation.toLocaleString()}</div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Yield Opportunities ({opportunities.length} found)</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            {loading ? (
              <div className="text-center text-muted-foreground py-8">Scanning protocols...</div>
                  ...
                  <CardContent className="pt-4">
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="text-2xl">{getStrategyIcon(opp.strategy)}</span>
                          <div>
                            <div className="font-semibold">{opp.protocol}</div>
                            <div className="text-xs text-muted-foreground">{opp.strategy}</div>
                          </div>
                        </div>
                        <div className="text-right">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        <div>
                          ...
                          <span className="font-semibold">${(opp.tvl / 1e9).toFixed(2)}B</span>
                        </div>
                        <div>
                          <span className="text-muted-foreground">Risk:</span>{" "}
                          <span className={`font-semibold ${getRiskColor(opp.riskScore)}`}>
                            {opp.riskScore}
                          </span>
                        </div>
                        <div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Badge variant="outline">Total: ${totalDeposited.toLocaleString()}</Badge>
              <Badge variant="default">Earned: ${totalEarnings.toFixed(2)}</Badge>
            </div>
          </CardTitle>
        </CardHeader>
        <CardContent>
            ...
            {positions.length === 0 ? (
              ...
              <>
                <div className="p-3 bg-muted rounded-lg">
                  ...
                  <div className="text-2xl font-bold text-green-600">{weightedAPY.toFixed(2)}%</div>
                </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="text-sm">
                          ...
                          <div className="flex justify-between">
                            ...
                            <span>{position.depositedAt.toLocaleDateString()}</span>
                          </div>
                        </div>
                      </div>
                    </CardContent>
