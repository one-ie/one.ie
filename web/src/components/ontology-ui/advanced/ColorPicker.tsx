...
export function ColorPicker({
  ...
  const useEyedropper = async () => {
    if ("EyeDropper" in window) {
      try {
        // @ts-expect-error - EyeDropper API not in types yet
              ...
              {presets.map((preset) => (
                <button
                  ...
                  className={cn(
                    ...
                    color === preset ? "border-primary ring-2 ring-ring" : "border-transparent"
                  )}
              ...
              {savedColors.map((saved) => (
                <div key={saved} className="relative group">
                  <button
                    ...
                    className={cn(
                      "h-8 w-8 rounded border-2 transition-all hover:scale-110",
                      color === saved ? "border-primary ring-2 ring-ring" : "border-transparent"
                    )}
