...
export function DimensionNav({
  ...
  return (
    <nav className={cn("flex flex-col gap-1 p-2", className)} aria-label="Dimension navigation">
      {DIMENSION_ORDER.map((dimension, index) => {
        ...
        return (
          ...
          >
            ...
            {!collapsed && (
              <>
                <span className="flex-1 text-left font-medium">{meta.name}</span>
            ...
            <span className="sr-only">
              View {meta.name} ({meta.description}){count !== undefined && ` - ${count} items`}
