...
import { AlertCircle, Check, ChevronLeft, ChevronRight, FormInput } from "lucide-react";
...
import type { FormField, OntologyComponentProps } from "../types";
  ...
  return (
    <Card className={cn("max-w-2xl mx-auto", className)}>
            ...
            {steps.map((step, index) => (
              <div key={index} className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  />
                  {index < currentStep && <Check className="h-4 w-4 text-primary" />}
                </div>
                <p
                  className={cn(
                    "text-xs mt-1",
      ...
      <form onSubmit={handleSubmit}>
        <CardContent className="space-y-4">{currentStepFields.map(renderField)}</CardContent>
