...
import { cn, getThingTypeDisplay, getThingTypeIcon, groupBy } from "../utils";
...
export function SearchResults({
  ...
  if (results.length === 0) {
    return (
      <div className={cn("text-center p-12 text-muted-foreground", className)}>
        <div className="text-6xl mb-4">🔍</div>
        <h3 className="text-lg font-medium mb-2">No results found</h3>
        <p className="text-sm">Try adjusting your search query or browse categories</p>
      </div>
    );
  }

  // Group results by type if enabled
  const groupedResults = groupByType ? groupBy(results, "type") : { All: results };
            ...
            {typeResults.map((result) => (
              <Card
                ...
                className={cn(
                  ...
                  onResultClick && "cursor-pointer hover:shadow-md hover:border-primary"
                )}
                ...
                <CardHeader className="pb-3">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2 flex-1">
                      <span className="text-xl">{getThingTypeIcon(result.type as any)}</span>
