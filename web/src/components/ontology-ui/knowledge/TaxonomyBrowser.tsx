...
export function TaxonomyBrowser({ labels, things, className }: TaxonomyBrowserProps) {
  ...
  const filteredLabels = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(
        (label) =>
          label.label.toLowerCase().includes(query) || label.category?.toLowerCase().includes(query)
      );
    }
        ...
        <Tabs defaultValue="tree" className="w-full">
          ...
          <TabsContent value="cloud" className="mt-4">
            {filteredLabels.length > 0 ? (
              <TagCloud labels={filteredLabels} onTagClick={handleTagClick} maxTags={50} />
            ) : (
              <p className="text-sm text-muted-foreground text-center py-8">No labels found</p>
            )}
          </TabsContent>
