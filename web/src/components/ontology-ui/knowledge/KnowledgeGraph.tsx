...
import type { Connection, Label, Thing } from "../types";
...
export function KnowledgeGraph({
  ...
  useEffect(() => {
    ...
    nodes.forEach((node) => {
      ...
      ctx.fillText(
        node.thing.name.length > 20 ? node.thing.name.slice(0, 17) + "..." : node.thing.name,
        node.x,
      ...
      if (isHovered && node.labels.length > 0) {
        ...
        node.labels.slice(0, 2).forEach((label, idx) => {
          ctx.fillText(label.label, node.x, node.y + node.radius + 30 + idx * 12);
        });
      }
    });
