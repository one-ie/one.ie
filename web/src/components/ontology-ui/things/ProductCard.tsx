...
export function ProductCard({
  ...
  return (
    ...
    >
      ...
      <CardHeader>
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-2 flex-1">
            ...
            <CardTitle
              className={size === "sm" ? "text-base" : size === "lg" ? "text-2xl" : "text-lg"}
            >
              {product.name}
            </CardTitle>
          </div>
      ...
      <CardContent>
        <div className="flex items-center justify-between">
          {productPrice !== undefined ? (
            <div className="text-2xl font-bold text-primary">{formatCurrency(productPrice)}</div>
          ) : (
      ...
      <CardFooter>
        <Button className="w-full" onClick={handleAddToCart} disabled={!isInStock || !onAddToCart}>
