...
import { Download, Eye, EyeOff, Filter, Grid, List, Network, Search } from "lucide-react";
...
import { ConnectionGraph, NetworkGraph } from "../visualization";
...
export function OntologyExplorer({
  ...
  function renderItems(items: any[], dimension: Dimension) {
    if (!items || items.length === 0) {
      return <div className="text-center py-12 text-muted-foreground">No {dimension} found</div>;
    }
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {(Object.keys(DIMENSIONS) as Dimension[]).map((dimension) => (
              ...
              >
                ...
                {DIMENSIONS[dimension].icon} {DIMENSIONS[dimension].name} (
