...
import { ConnectionCard, EventCard, GroupCard, LabelCard, ThingCard, UserCard } from "../index";
import type { Connection, Dimension, Event, Group, Label, Person, Thing } from "../types";
...
export function ComponentToChat({
  ...
  function renderComponent() {
    ...
    const commonProps = {
      onClick: interactive ? () => onAction?.("click", component) : undefined,
    };

    switch (dimension) {
      case "things":
        return <ThingCard thing={data as Thing} type={(data as Thing).type} {...commonProps} />;
      case "people":
        return <UserCard user={data as Person} {...commonProps} />;
      case "events":
        return <EventCard event={data as Event} {...commonProps} />;
      case "connections":
        return <ConnectionCard connection={data as Connection} {...commonProps} />;
      case "groups":
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {componentState && <Badge variant="outline">Interactive</Badge>}
          </div>

          <div className="flex items-center gap-1">
            {/* Share button */}
            {shareable && (
              <Button variant="ghost" size="sm" onClick={copyShareLink} className="h-8 w-8 p-0">
                {copied ? (
      ...
      <CardContent>
        ...
        {interactive && (
          <div className="mt-4 flex gap-2">
            ...
            <Button variant="outline" size="sm" onClick={() => onAction?.("open", component)}>
              Open
            </Button>
          </div>
        )}
