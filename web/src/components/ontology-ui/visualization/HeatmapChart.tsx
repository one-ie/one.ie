...
export function HeatmapChart({
  ...
  const exportToCSV = () => {
    ...
    const csvContent = [headers.join(","), ...data.map((d) => `${d.x},${d.y},${d.value}`)].join(
      "\n"
    );
    ...
    <Card className="w-full">
      ...
      <CardContent>
        ...
        <div className="mt-4 flex items-center justify-center gap-4">
          <span className="text-sm text-muted-foreground">{dataMin.toFixed(1)}</span>
          ...
          <span className="text-sm text-muted-foreground">{dataMax.toFixed(1)}</span>
        </div>
      </CardContent>
    </Card>
  );
