...
import type { Connection, ConnectionType, FormProps, Thing } from "../types";
...
export function ConnectionCreator({
  ...
  const [metadata, setMetadata] = useState<Record<string, string>>(initialData?.metadata || {});
            ...
            <CommandGroup className="max-h-64 overflow-auto">
              {things.map((thing) => (
                ...
                >
                  ...
                  <span className="text-xs text-muted-foreground ml-2">{thing.type}</span>
                </CommandItem>
              ))}
            </CommandGroup>
          </Command>
  ...
  return (
    <Card className={className}>
      <CardHeader>
        <CardTitle>{mode === "create" ? "Create Connection" : "Edit Connection"}</CardTitle>
      </CardHeader>
          ...
          {Object.entries(metadata).length > 0 && (
            <div className="space-y-2">
              {Object.entries(metadata).map(([key, value]) => (
                <div key={key} className="flex items-center gap-2 p-2 bg-secondary rounded-md">
                  <span className="text-sm font-medium flex-1">
                    {key}: {value}
                  </span>
                  <Button variant="ghost" size="sm" onClick={() => handleRemoveMetadata(key)}>
                    ×
                  </Button>
                </div>
              ))}
