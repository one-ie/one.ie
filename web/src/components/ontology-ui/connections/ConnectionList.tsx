...
export function ConnectionList({
  ...
  const connectionTypes = Array.from(new Set(connections.map((c) => c.type))).sort();
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {sortable && (
              <>
                ...
                <Button
                  variant={sortConfig?.field === "strength" ? "default" : "outline"}
                  size="sm"
                  onClick={() => setSort("strength")}
                >
                  Strength{" "}
                  {sortConfig?.field === "strength" && (sortConfig.direction === "asc" ? "↑" : "↓")}
                </Button>
              </>
            )}
          </div>
