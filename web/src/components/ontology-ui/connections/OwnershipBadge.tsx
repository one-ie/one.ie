...
const sizeClasses: Record<
  OwnershipBadgeSize,
  { badge: string; avatar: string; text: string; icon: string }
> = {
  sm: {
...
export function OwnershipBadge({
  ...
  return (
    <Badge
      variant="secondary"
      className={cn("inline-flex items-center font-medium", sizes.badge, className)}
    >
      ...
      <span className={cn("truncate", sizes.text)}>{owner.name}</span>
    </Badge>
  );
}
