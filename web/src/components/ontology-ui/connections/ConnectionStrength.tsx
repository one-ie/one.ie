    ...
    <div className={cn("space-y-2", className)}>
      ...
      {(showLabel || showPercentage) && (
        <div className="flex items-center justify-between gap-2">
          ...
          {showPercentage && (
            <span className={cn("text-sm font-medium", colors[level].text)}>{strength}%</span>
          )}
        </div>
      )}
      ...
      <div className="relative">
        <Progress
          value={strength}
          className={cn("w-full", sizeClasses[size], "transition-all duration-300")}
      ...
      {connection.metadata?.strengthReason && (
        <p className="text-xs text-muted-foreground">{connection.metadata.strengthReason}</p>
      )}
    </div>
  );
}
...
export function ConnectionStrengthBadge({ strength, className }: ConnectionStrengthBadgeProps) {
  const getStrengthLevel = (value: number): "weak" | "medium" | "strong" => {
    if (value < 33) return "weak";
    if (value < 67) return "medium";
    return "strong";
  ...
  const colors = {
    ...
    medium:
      "bg-yellow-100 text-yellow-800 border-yellow-500 dark:bg-yellow-900 dark:text-yellow-100",
  ...
  return (
    <Badge variant="outline" className={cn("capitalize", colors[level], className)}>
