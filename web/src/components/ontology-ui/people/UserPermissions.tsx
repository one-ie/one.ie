...
import type { Permission, Person } from "../types";
    ...
    <Card className={cn("w-full", className)}>
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Badge className={roleColorClass}>{getRoleDisplay(user.role)}</Badge>
        </div>
          ...
          {resources.length === 0 ? (
            ...
            resources.map((resource) => (
              <div key={resource} className="space-y-3">
                <h4 className="text-sm font-semibold capitalize">{resource.replace(/_/g, " ")}</h4>
