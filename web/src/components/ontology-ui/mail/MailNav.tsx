...
export function MailNav({
  ...
  const [dragOverFolder, setDragOverFolder] = React.useState<MailFolder | null>(null);
  ...
  const renderLink = (link: NavLink) => {
        ...
        <Tooltip key={link.folder} delayDuration={0}>
          <TooltipTrigger asChild>
            <Button
              ...
              size="icon"
              className={cn("size-9", dragOverFolder === link.folder && "bg-accent")}
          ...
          <TooltipContent side="right" className="flex items-center gap-4">
            ...
            {link.count !== undefined && link.count > 0 && (
              <span className="ml-auto text-muted-foreground">{link.count}</span>
            )}
          </TooltipContent>
        </Tooltip>
      );
    ...
    return (
      <DropdownMenu key={link.folder}>
        <Button
          variant={link.variant}
          className={cn("w-full justify-start", dragOverFolder === link.folder && "bg-accent")}
          onClick={() => onFolderChange(link.folder)}
          onDragOver={(e) => handleDragOver(e, link.folder)}
          onDragLeave={handleDragLeave}
          onDrop={(e) => handleDrop(e, link.folder)}
  ...
  return (
    <div className="flex h-full flex-col gap-2 p-2">
      {/* Primary folders */}
      <nav className="grid gap-1 group">{primaryLinks.map(renderLink)}</nav>

      <Separator />

      {/* Category folders */}
      <nav className="grid gap-1 group">{categoryLinks.map(renderLink)}</nav>
