...
import { Archive, ChevronDown, Mail, MailOpen, Star, Trash2 } from "lucide-react";
      ...
      >
        ...
        {showAvatars && !bulkMode && (
          <div className="flex size-8 items-center justify-center rounded-full bg-muted flex-shrink-0">
            {email.from.avatar ? (
              ...
              <span className="text-xs font-medium">{email.from.name.charAt(0).toUpperCase()}</span>
            )}
          </div>
        )}
        ...
        <div className="flex-1 space-y-1 overflow-hidden">
          <div className="flex items-center gap-2">
            ...
            {!email.read && <span className="flex size-2 rounded-full bg-blue-600" />}
            <span
              className={cn(
          ...
          <div className={cn("text-xs font-medium line-clamp-1", email.read && "font-normal")}>
            {email.subject}
          </div>
          <div className="line-clamp-2 text-xs text-muted-foreground">{email.preview}</div>
        ...
        {!bulkMode && (
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
            ...
            >
              <Star className={cn("size-4", email.starred && "fill-yellow-400 text-yellow-400")} />
            </Button>
            <Button
              variant="ghost"
              size="icon"
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center gap-2 border-b p-2">
        ...
        {allowBulkSelect && (
          <Button variant={bulkMode ? "default" : "ghost"} size="sm" onClick={toggleBulkMode}>
            Select
          </Button>
        )}
        ...
        {bulkMode && selectedIds.length > 0 && (
          <>
            ...
            <Button variant="ghost" size="sm" onClick={() => handleBulkMarkRead(true)}>
              <MailOpen className="mr-1 size-4" />
              Mark read
            </Button>
            <Button variant="ghost" size="sm" onClick={() => handleBulkMarkRead(false)}>
        ...
        {bulkMode && (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuItem
                onClick={() => onSelectionChange?.(emails.filter((e) => e.read).map((e) => e.id))}
              >
                Read
              </DropdownMenuItem>
              <DropdownMenuItem
              ...
              <DropdownMenuItem
                onClick={() =>
                  onSelectionChange?.(emails.filter((e) => e.starred).map((e) => e.id))
                }
              >
                Starred
              </DropdownMenuItem>
        ...
        {!bulkMode && onSortChange && (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onClick={() => onSortChange("subject", "desc")}>
                Subject (Z-A)
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
