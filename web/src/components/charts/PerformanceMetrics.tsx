...
import { Zap, Globe, Gauge, DollarSign, Sparkles, TrendingUp } from 'lucide-react';
    ...
    <div className="space-y-2">
      <div className="flex h-2 w-full overflow-hidden rounded-full bg-muted gap-0.5">
        <div
          ...
          style={{ transform: `scaleX(${progress[0] / 100})`, transformOrigin: 'left' }}
        />
        <div
          className="w-1/4 bg-gradient-to-r from-purple-600 to-pink-500 transition-all duration-700 delay-300 ease-out"
          style={{ transform: `scaleX(${progress[1] / 100})`, transformOrigin: 'left' }}
        />
        <div
          className="flex-1 bg-gradient-to-r from-orange-600 to-red-500 transition-all duration-500 delay-600 ease-out"
          style={{ transform: `scaleX(${progress[2] / 100})`, transformOrigin: 'left' }}
        />
      </div>
      <p className="text-xs text-muted-foreground text-center">Build • Upload • Deploy</p>
    </div>
  );
}
...
function CostVisualization() {
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-xs text-muted-foreground text-center font-medium">ONE vs Competitors</p>
    </div>
  );
}
    ...
    <div className="w-full space-y-8">
      ...
      <Card className="border-primary/20 bg-gradient-to-br from-primary/5 via-background to-background">
        <CardHeader>
          ...
          <CardDescription>
            Every metric translates to a better user experience and lower costs for you
          </CardDescription>
        </CardHeader>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground ml-4">
                Iterate faster. Ship bug fixes and features in seconds, not minutes.
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground ml-4">
                Users worldwide experience sub-300ms response times. Faster = higher conversion.
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground ml-4">
                SEO friendly. Accessibility compliant. Performance optimized for all devices.
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground ml-4">
                No surprise bills. Scale infinitely. Same price whether you have 1 user or 1M.
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <Card className="border-border/50 bg-card/50 backdrop-blur">
        ...
        <CardContent>
          <div className="space-y-4">
            {[
              { platform: 'Vercel', deploy: '45s', latency: '340ms', cost: '$229/mo', lighthouse: '95' },
              { platform: 'Netlify', deploy: '60s', latency: '365ms', cost: '$240/mo', lighthouse: '92' },
              { platform: 'AWS', deploy: '120s', latency: '420ms', cost: '$350/mo', lighthouse: '88' },
              { platform: 'ONE Platform', deploy: '19s', latency: '287ms', cost: '$0/mo', lighthouse: '100' },
            ].map((row, idx) => (
              <div
                key={idx}
              ...
              >
                ...
                <div className="text-sm text-muted-foreground">{row.deploy}</div>
                <div className="text-sm text-muted-foreground">{row.latency}</div>
                ...
                <div className={`text-sm ${
                  row.lighthouse === '100' ? 'text-green-600 font-semibold' : 'text-muted-foreground'
                }`}>
                  {row.lighthouse}
                </div>
              </div>
