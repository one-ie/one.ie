...
import { DollarSign, Gauge, Globe, Sparkles, TrendingUp, Zap } from "lucide-react";
      ...
      <CardContent className="relative space-y-4">
        ...
        <div className="space-y-1">
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-lg text-muted-foreground font-medium">{metric.unit}</span>
          </div>
        ...
        {/* Description */}
        {showDescription && <p className="text-sm text-muted-foreground">{metric.description}</p>}
    ...
    <div className="flex gap-1.5">
      {["Perf", "A11y", "BP", "SEO"].map((badge, idx) => (
        <div
          ...
          style={{ animationDelay: `${idx * 100}ms` }}
        >
          <span className="text-xs font-bold text-green-700 dark:text-green-400">{badge}</span>
        </div>
      ))}
    </div>
  );
          ...
          <div className="space-y-4">
            {[
              ...
              >
                ...
                <div
                  className={`font-medium text-sm ${row.cost === "$0/mo" ? "text-green-600" : ""}`}
                >
                  {row.cost}
                </div>
                <div
