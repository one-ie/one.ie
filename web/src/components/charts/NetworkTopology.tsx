...
function NetworkMap({
	...
	const [connections, setConnections] =
		useState<{ from: string; to: string; active: boolean }[]>(
			defaultConnections,
		);
							...
							<div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 opacity-0 group-hover/node:opacity-100 transition-opacity duration-300 pointer-events-none z-10">
								<div className="bg-background border border-border rounded-lg shadow-xl p-2 whitespace-nowrap">
									...
									<p className="text-xs text-muted-foreground">
										{node.latency}ms • {(node.requests / 1000).toFixed(1)}k
										req/hr
									</p>
								</div>
							</div>
...
export function NetworkTopology({
	...
	const highLoadNodes = edgeNodes.filter(
		(n) => n.status === "high-load",
	).length;
						...
						<div className="grid gap-6 md:grid-cols-2">
							<div className="space-y-3">
								...
								<p className="text-sm text-muted-foreground">
									Deploy once, serve everywhere. Your content is automatically
									distributed to all edge locations worldwide.
								</p>
							</div>
							<div className="space-y-3">
								<h4 className="font-semibold flex items-center gap-2">
								...
								<p className="text-sm text-muted-foreground">
									Users experience sub-50ms response times from the nearest
									edge, improving conversion rates by up to 23%.
								</p>
							</div>
							<div className="space-y-3">
								<h4 className="font-semibold flex items-center gap-2">
								...
								<p className="text-sm text-muted-foreground">
									Traffic automatically routes to available nodes. Handle viral
									traffic spikes without any configuration.
								</p>
							</div>
							<div className="space-y-3">
								<h4 className="font-semibold flex items-center gap-2">
								...
								<p className="text-sm text-muted-foreground">
									Redundant infrastructure ensures your site stays online even
									if entire regions go offline.
								</p>
							</div>
						</div>
					</CardContent>
