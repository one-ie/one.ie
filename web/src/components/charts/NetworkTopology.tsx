...
import { Activity, Globe2, MapPin, Server, Signal, Wifi } from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-start justify-between mb-4">
        ...
        <Badge
          variant="outline"
          className={`text-xs font-bold ${statusBadgeColors[node.status]} animate-pulse`}
        >
...
function NetworkMap({ nodes, animate }: { nodes: EdgeNode[]; animate: boolean }) {
  ...
  const [connections, setConnections] =
    useState<{ from: string; to: string; active: boolean }[]>(defaultConnections);

  useEffect(() => {
    if (animate && typeof window !== "undefined") {
      // Only animate connections on client side
      const interval = setInterval(() => {
        setConnections((prev) => prev.map((conn) => ({ ...conn, active: Math.random() > 0.3 })));
      }, 2000);
      ...
      {nodes.map((node) => {
        ...
        return (
          ...
          >
            ...
            {/* Node Dot */}
            <div
              className={`relative h-4 w-4 rounded-full border-2 shadow-lg transition-all duration-300 hover:scale-150 ${
                node.status === "high-load"
              ...
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 opacity-0 group-hover/node:opacity-100 transition-opacity duration-300 pointer-events-none z-10">
                <div className="bg-background border border-border rounded-lg shadow-xl p-2 whitespace-nowrap">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {node.latency}ms • {(node.requests / 1000).toFixed(1)}k req/hr
                  </p>
                </div>
              </div>
            </div>
...
export function NetworkTopology({
  ...
  const avgLatency = Math.round(
    edgeNodes.reduce((sum, node) => sum + node.latency, 0) / edgeNodes.length
  );
    ...
    <div className="w-full space-y-8">
      ...
      <Card className="relative overflow-hidden border-2 border-primary/30 bg-gradient-to-br from-primary/10 via-background to-background shadow-2xl">
        ...
        <CardContent className="relative space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-4">
            <div className="rounded-lg border border-border/50 bg-background/50 backdrop-blur-xl p-4 space-y-1">
              ...
              <p className="text-2xl font-black text-primary">
                {(totalRequests / 1000).toFixed(0)}k/hr
              </p>
            </div>
            <div className="rounded-lg border border-border/50 bg-background/50 backdrop-blur-xl p-4 space-y-1">
          ...
          {/* View Mode Toggle */}
          {viewMode === "map" && <NetworkMap nodes={edgeNodes} animate={animate} />}
        </CardContent>
      </Card>
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {edgeNodes.map((node, idx) => (
              <EdgeNodeCard key={node.id} node={node} index={idx} animate={animate} />
            ))}
          </div>
        </div>
      )}
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <div className="space-y-3">
                ...
                <p className="text-sm text-muted-foreground">
                  Deploy once, serve everywhere. Your content is automatically distributed to all
              ...
              <div className="space-y-3">
                ...
                <p className="text-sm text-muted-foreground">
                  Users experience sub-50ms response times from the nearest edge, improving
                  conversion rates by up to 23%.
                </p>
              </div>
              <div className="space-y-3">
                <h4 className="font-semibold flex items-center gap-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Traffic automatically routes to available nodes. Handle viral traffic spikes
              ...
              <div className="space-y-3">
                ...
                <p className="text-sm text-muted-foreground">
                  Redundant infrastructure ensures your site stays online even if entire regions go
                  offline.
                </p>
              </div>
            </div>
