...
function ResourceCard({
	...
	if (viewMode === "compact") {
		return (
			...
			>
				<div
					className={`p-2 rounded-lg bg-gradient-to-br ${resource.color} text-white`}
				>
					{resource.icon}
				</div>
...
export function ResourceUsage({
	...
	const totalUsage =
		resources.reduce((sum, r) => sum + (r.current / r.max) * 100, 0) /
		resources.length;
	const criticalResources = resources.filter(
		(r) => r.status === "critical",
	).length;
	const warningResources = resources.filter(
		(r) => r.status === "warning",
	).length;
