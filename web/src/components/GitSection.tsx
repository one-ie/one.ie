...
export function GitSection({ children }: GitSectionProps) {
			...
			try {
				...
				if (
					...
					const nextStats = {
						...
						downloads:
							npmData && typeof npmData.downloads === "number"
								? npmData.downloads
								: 0,
					};
	...
	useEffect(() => {
		...
		const step = (timestamp: number) => {
			...
			setDisplayStats({
				stars: Math.round(
					initial.stars + (target.stars - initial.stars) * progress,
				),
		...
		if (
			target.stars !== initial.stars ||
			target.downloads !== initial.downloads
		) {
				...
				<div className="text-center space-y-6">
					...
					<div className="space-y-4">
						<p className="text-lg sm:text-xl text-muted-foreground max-w-2xl mx-auto">
							Build apps, websites, and AI agents is English. Download to your
							computer, run in the cloud and deploy to the edge or your own
							server. ONE is open source and free forever.
						</p>
				...
				<div className="grid gap-6 md:grid-cols-3">
					...
					<Card className="group relative overflow-hidden border-border/50 hover:shadow-2xl transition-all duration-500">
						<div
							...
							style={{
								background:
									"linear-gradient(135deg, hsl(45 93% 47% / 0.03) 0%, transparent 100%)",
							}}
						/>
						...
						<div className="relative p-8 space-y-6">
							...
							<div className="relative">
								<Button
									...
									onClick={() =>
										copyCommand(`git clone ${GITHUB_URL}.git`, "git")
									}
				...
				<div className="text-center py-4">
					<a href="/news/over-10000-installs" className="block">
						<p className="text-2xl font-semibold text-foreground hover:opacity-80 transition-opacity">
							Over <span style={{ color: "hsl(45 93% 47%)" }}>10,000</span>{" "}
							Installs
						</p>
					</a>
					<a
