    ...
    <div className="space-y-1.5">
      {entries
        ...
        .map((entry) => (
          <Card
            key={entry.id}
            className="group hover:shadow-sm transition-all duration-300 border-muted/30 hover:border-primary/30 bg-background/80 backdrop-blur-sm py-2 px-3"
          >
            <a href={`/docs/${entry.slug}`} className="no-underline block">
