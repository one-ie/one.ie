...
export function TelegramMessage({ message, isUser = false }: TelegramMessageProps) {
  const profile = message.sender in AGENT_PROFILES
    ? AGENT_PROFILES[message.sender as keyof typeof AGENT_PROFILES]
    : null;
  ...
  return (
    ...
    >
      ...
      <div className={cn("flex flex-col", isUser ? "items-end" : "items-start", "max-w-[80%]")}>
        ...
        {!isUser && (
          <div className="text-base font-medium text-muted-foreground mb-1 px-2">
            {senderName}
          </div>
        )}
              ...
              {message.metadata.toolName && (
                <Badge variant="secondary" className="mb-2 text-xs">
                  ...
                  {message.metadata.duration && (
                    <span className="ml-1">({message.metadata.duration.toFixed(2)}s)</span>
                  )}
                </Badge>
              )}
