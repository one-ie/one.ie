...
const TOOLS: Tool[] = [
	{
		...
		instructions: (
			<ol className="space-y-2 text-sm leading-relaxed text-muted-foreground">
				...
				<li>
					3. Add any repository notes, then press Cmd/Ctrl + Enter to start.
				</li>
			</ol>
		),
	},
	{
		...
		instructions: (
			<ol className="space-y-2 text-sm leading-relaxed text-muted-foreground">
				<li>
					1. Open the OpenAI Playground and choose the latest GPT-4o or GPT-4
					model.
				</li>
				<li>
					2. Paste the prompt into the input area with ``` fences if desired.
				</li>
				<li>
					3. Press Submit or hit Cmd/Ctrl + Enter to generate the response.
				</li>
			</ol>
		),
	},
	...
	},
	{
		...
		instructions: (
			<ol className="space-y-2 text-sm leading-relaxed text-muted-foreground">
				<li>
					1. Launch Windsurf and open the Command Center (Cmd/Ctrl + Shift + L).
				</li>
	...
	},
	{
		...
		instructions: (
			<ol className="space-y-2 text-sm leading-relaxed text-muted-foreground">
				...
				<li>
					2. Paste the prompt, optionally prefixing with “You are an
					ontology-aligned engineer.”
				</li>
				<li>
					3. Ask for code, docs, or design support and refine iteratively.
				</li>
			</ol>
		),
	},
...
export function SendToTools({
	...
	const [copiedState, setCopiedState] = useState<"snippet" | "brief" | null>(
		null,
	);
	...
	const copy = async (text: string, key: "snippet" | "brief") => {
		try {
			...
			setCopiedState(key);
			setTimeout(
				() => setCopiedState((current) => (current === key ? null : current)),
				2000,
			);
		} catch (error) {
						...
						<div className="flex items-start justify-between gap-4 border-b border-border/60 p-6">
							<div className="space-y-2">
								...
								<p className="text-sm text-muted-foreground">
									Paste the brief into {activeTool.label} and follow these steps
									to get started.
								</p>
							</div>
							<Button
