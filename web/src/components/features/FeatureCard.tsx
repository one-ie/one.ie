...
const statusHoverColors = {
  ...
  in_development: "hover:border-yellow-200 hover:shadow-lg hover:shadow-yellow-500/5",
...
export function FeatureCard({
  ...
  const category = feature.data.category as keyof typeof categoryColors | undefined;
  const categoryClass = category ? categoryColors[category] || categoryColors.other : null;

  const displayStatus =
    status.replace("-", " ").charAt(0).toUpperCase() + status.replace("-", " ").slice(1);

  return (
    <a href={`/features/${feature.slug}`} className={`block transition-all ${className}`}>
      <Card
        ...
        <CardHeader className="space-y-4">
          <div className="flex items-start justify-between gap-2 flex-wrap">
            ...
            {showCategory && categoryClass && (
              <Badge variant="outline" className={`capitalize text-xs ${categoryClass}`}>
                {feature.data.category?.replace("-", " ")}
              </Badge>
            )}
          </div>
          <div className="space-y-2">
            <CardTitle className="text-xl line-clamp-2">{feature.data.title}</CardTitle>
        ...
        <CardContent>
          <div className="space-y-3">
            ...
            {feature.data.version && (
              <div className="text-sm text-muted-foreground">v{feature.data.version}</div>
            )}
