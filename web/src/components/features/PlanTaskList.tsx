...
const DEFAULT_PHASES = [
	{
		...
		description: "Validate requirements and plan",
	},
	{
		range: "11-20",
	...
	},
	{
		range: "51-60",
		phase: "Knowledge & RAG",
		description: "Create embeddings and search",
	},
	{
		range: "61-70",
	...
	},
	{
		...
		description: "Optimize and profile",
	},
	{
		range: "91-100",
...
export function PlanTaskList({
	...
	const [taskStatuses, setTaskStatuses] = useState<Record<number, string>>(
		tasks.reduce(
			(acc, task) => ({ ...acc, [task.cycleNumber]: task.status }),
			{},
		),
	);
	...
	const tasksByPhase = useMemo(() => {
		return phases.map((phase) => {
			...
			const phaseTasks = tasks.filter(
				(t) => t.cycleNumber >= start && t.cycleNumber <= end,
			);
			return {
				phase,
				tasks: phaseTasks,
				completed: phaseTasks.filter(
					(t) => taskStatuses[t.cycleNumber] === "completed",
				).length,
			};
		});
		...
		<div className="space-y-4">
			{tasksByPhase.map((item) => {
				...
				const phaseProgress = Math.round(
					(item.completed / item.tasks.length) * 100,
				);
				const isExpanded = expandedPhases.has(item.phase.range);

				return (
					<div
						key={item.phase.range}
							...
							<div className="divide-y">
								{item.tasks.map((task) => {
									const Icon = getStatusIcon(
										taskStatuses[task.cycleNumber] || task.status,
									);
									const currentStatus =
										taskStatuses[task.cycleNumber] || task.status;
									...
									return (
										<div
											...
											onClick={() =>
												editable && handleTaskToggle(task.cycleNumber)
											}
										>
				...
				<div className="grid grid-cols-3 gap-4 mt-6 pt-4 border-t">
					<div className="text-center">
						<div className="text-2xl font-bold text-green-600">
							{
								tasks.filter((t) => taskStatuses[t.cycleNumber] === "completed")
									.length
							}
						</div>
					...
					<div className="text-center">
						<div className="text-2xl font-bold text-blue-600">
							{
								tasks.filter(
									(t) => taskStatuses[t.cycleNumber] === "in_progress",
								).length
							}
						</div>
					...
					<div className="text-center">
						<div className="text-2xl font-bold text-gray-600">
							{
								tasks.filter((t) => taskStatuses[t.cycleNumber] === "pending")
									.length
							}
						</div>
