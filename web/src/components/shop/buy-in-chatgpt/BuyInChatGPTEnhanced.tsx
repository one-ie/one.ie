...
interface ProductChatAssistantEnhancedProps {
	product?: {
		id: string;
		name: string;
		price: number;
...
export function ProductChatAssistantEnhanced({
	product,
}: ProductChatAssistantEnhancedProps) {
...
Be friendly, knowledgeable, and concise.`
	...
	const handleSessionCreated = async (sessionId: string) => {
		...
		const aiResponse: Message = {
			...
			role: "assistant",
			content:
				"Great! Now I just need your shipping address to calculate shipping and tax.",
			timestamp: new Date(),
		};
	...
	const handleAddressSubmitted = (updatedSession: any) => {
		console.log(
			"[ProductChat] Address submitted, updated session:",
			updatedSession,
		);
						...
						<div className="flex justify-start">
							<div className="max-w-[85%] space-y-3">
								...
								<PaymentProcessor
									sessionId={checkoutSession.id}
									totalAmount={
										checkoutSession.totals.find((t) => t.type === "total")
											?.amount || 0
									}
