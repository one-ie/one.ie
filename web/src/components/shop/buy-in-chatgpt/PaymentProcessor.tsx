...
export function PaymentProcessor({
	...
	const generateTestSpt = async () => {
		...
		try {
			...
			setError(
				err instanceof Error ? err.message : "Failed to generate test token",
			);
		} finally {
					...
					<div className="bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 p-4 rounded-md space-y-3">
						<div>
							...
							<p className="text-xs text-blue-800 dark:text-blue-200 leading-relaxed">
								<strong>In Production:</strong> When a customer shops in
								ChatGPT, ChatGPT automatically creates a Shared Payment Token
								(SPT) using the customer saved payment method. The SPT is sent
								to us and we charge it via Stripe.
							</p>
						</div>
						<div className="pt-2 border-t border-blue-200 dark:border-blue-700">
							<p className="text-xs text-blue-800 dark:text-blue-200 leading-relaxed">
								<strong>For This Demo:</strong> We will create a real Stripe
								test transaction using a test card. Click below to generate a
								test payment method - this will appear in your Stripe dashboard.
							</p>
						</div>
					</div>
