...
export function ProductChatAssistant() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      role: 'assistant',
      content: "Hi! I'm your AI shopping assistant. I can help you learn more about this product, compare features, or answer any questions you have. What would you like to know?",
      timestamp: new Date(),
    },
  ]);
  ...
  useEffect(() => {
    try {
      ...
      for (const el of Array.from(priceElements)) {
        ...
        if (val > 10 && price === 0) { // First significant price found
          price = val;
          break;
        }
      }
      ...
      let brand = '';
      const brandSpec = Array.from(document.querySelectorAll('span')).find(
        el => el.textContent?.toLowerCase().includes('brand')
      );
      if (brandSpec) {
  ...
  const handleSend = async (messageText?: string) => {
    ...
    try {
      ...
      const featuresText = productContext?.features && productContext.features.length > 0
        ? `\nKey Features:\n${productContext.features.map(f => `- ${f}`).join('\n')}`
        : '';

      const systemPrompt = productContext ? `You are a helpful shopping assistant for an e-commerce store. You're currently helping a customer who is viewing this product:
        ...
        <div className="space-y-4">
          {messages.map((message) => (
            <div key={message.id}>
              ...
              {/* Show Add to Cart card if this is a purchase-related message */}
              {message.role === 'assistant' && message.showAddToCart && productContext && (
...
export default ProductChatAssistant;
