			...
			<Card className="overflow-hidden">
				...
				{data.image && (
					<div className="relative aspect-square overflow-hidden bg-foreground/5">
						...
						{data.badge && (
							<Badge className="absolute top-3 left-3 bg-tertiary text-white">
								{data.badge}
							</Badge>
						)}
					</div>
				...
				<CardHeader>
					<div className="flex items-start justify-between gap-2">
						<div className="flex-1">
							<CardTitle className="text-font line-clamp-2">
								{data.title}
							</CardTitle>
							{data.category && (
				...
				<CardContent className="space-y-4">
					...
					{data.stock !== undefined && (
						<div className="flex items-center gap-2 text-sm">
							...
							<span
								className={`text-font/80 ${data.stock < 10 ? "text-destructive" : ""}`}
							>
				...
				<CardFooter className="flex flex-col gap-2">
					...
					<div className="flex w-full items-center gap-2">
						...
						<div className="flex items-center border border-border rounded-md bg-background">
							...
							<Button
								...
								size="sm"
								onClick={() =>
									setQuantity(Math.min(data.stock || 999, quantity + 1))
								}
								className="h-8 w-8 p-0 text-font"
					...
					<Button
						...
						disabled={
							addedToCart || (data.stock !== undefined && data.stock === 0)
						}
					>
						...
						<CardContent className="p-4">
							<div className="flex items-center gap-3">
								...
								<p className="text-sm font-medium">
									<span className="font-semibold">{data.title}</span> has been
									added to your cart
								</p>
							</div>
						</CardContent>
