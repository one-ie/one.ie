...
const ONTOLOGY_DIMENSIONS = ["groups", "people", "things", "connections", "events", "knowledge"];
      ...
      describe(`Feature: ${feature.data.title}`, () => {
        it("should map to at least one ontology dimension", () => {
          if (feature.data.ontologyMapping) {
            const mappedDimensions = Object.entries(feature.data.ontologyMapping)
      ...
      describe(`Feature: ${feature.data.title}`, () => {
        it("should document groups scoping if applicable", () => {
          if (feature.data.ontologyMapping?.groups) {
            expect(feature.data.ontologyMapping.groups.length).toBeGreaterThan(0);
          }
        });
        ...
        it("should explain multi-tenant isolation if present", () => {
          ...
          if (
            ...
            expect(feature.data.ontologyMapping.groups).toMatch(/group|organization|tenant|scope/i);
          }
        });
      ...
      describe(`Feature: ${feature.data.title}`, () => {
        it("should document people/roles if authorization exists", () => {
          if (feature.data.ontologyMapping?.people) {
            expect(feature.data.ontologyMapping.people.length).toBeGreaterThan(0);
          }
        });
        ...
        it("should document role requirements if present", () => {
          ...
          if (
            ...
            expect(feature.data.ontologyMapping.people).toMatch(/role|permission|owner|user/i);
          }
        });
        ...
        it("should mention actor identity in event logging", () => {
          ...
          if (feature.data.ontologyDimensions?.includes("events")) {
            if (feature.data.ontologyMapping?.events) {
              expect(feature.data.ontologyMapping.events).toMatch(/actor|user|event|log/i);
            }
          }
        });
      ...
      describe(`Feature: ${feature.data.title}`, () => {
        it("should document thing types if applicable", () => {
          if (feature.data.ontologyMapping?.things) {
            expect(feature.data.ontologyMapping.things.length).toBeGreaterThan(0);
          }
        });
        ...
        it("should explain what entities are created/modified", () => {
          ...
          if (
            ...
            expect(feature.data.ontologyMapping.things).toMatch(/entity|thing|type|object/i);
          }
        });
        ...
        it("should reference thing types in use cases", () => {
          if (feature.data.useCases && feature.data.ontologyDimensions?.includes("things")) {
      ...
      describe(`Feature: ${feature.data.title}`, () => {
        it("should document relationships if they exist", () => {
          if (feature.data.ontologyMapping?.connections) {
            expect(feature.data.ontologyMapping.connections.length).toBeGreaterThan(0);
          }
        });
    ...
    allFeatures.forEach((feature) => {
      describe(`Feature: ${feature.data.title}`, () => {
        it("should document audit trail requirements", () => {
          if (feature.data.ontologyMapping?.events) {
            expect(feature.data.ontologyMapping.events.length).toBeGreaterThan(0);
          }
        });
        ...
        it("should list all events that are logged", () => {
          ...
          if (
            ...
            expect(feature.data.ontologyMapping.events).toMatch(/event|log|action|audit/i);
          }
        });
        ...
        it("should document event retention policy if applicable", () => {
          if (feature.data.tags?.includes("audit") && feature.data.ontologyMapping?.events) {
            expect(feature.data.ontologyMapping.events.length).toBeGreaterThan(0);
          }
        });
      });
      ...
      describe(`Feature: ${feature.data.title}`, () => {
        it("should document RAG/search requirements if applicable", () => {
          if (feature.data.ontologyMapping?.knowledge) {
            expect(feature.data.ontologyMapping.knowledge.length).toBeGreaterThan(0);
          }
        });
    ...
    allFeatures.forEach((feature) => {
      describe(`Feature: ${feature.data.title}`, () => {
        it("should have consistent ontology dimensions across fields", () => {
          ...
          if (feature.data.ontologyMapping?.groups && feature.data.ontologyDimensions) {
        ...
        it("should document all used dimensions", () => {
          ...
          const usedInText = ONTOLOGY_DIMENSIONS.filter((dim) => {
            const text = [
              ...
              feature.data.ontologyMapping?.[dim as keyof typeof feature.data.ontologyMapping],
            ]
              .join(" ")
              .toLowerCase();
  ...
  describe("Ontology Coverage", () => {
    it("should have features using groups dimension", () => {
      const withGroups = allFeatures.filter(
        (f) => f.data.ontologyMapping?.groups || f.data.ontologyDimensions?.includes("groups")
      );
    ...
    it("should have features using people dimension", () => {
      const withPeople = allFeatures.filter(
        (f) => f.data.ontologyMapping?.people || f.data.ontologyDimensions?.includes("people")
      );
    ...
    it("should have features using things dimension", () => {
      const withThings = allFeatures.filter(
        (f) => f.data.ontologyMapping?.things || f.data.ontologyDimensions?.includes("things")
      );
    ...
    it("should have features using events dimension", () => {
      const withEvents = allFeatures.filter(
        (f) => f.data.ontologyMapping?.events || f.data.ontologyDimensions?.includes("events")
      );
    ...
    it("should have features using knowledge dimension", () => {
      const withKnowledge = allFeatures.filter(
        (f) => f.data.ontologyMapping?.knowledge || f.data.ontologyDimensions?.includes("knowledge")
      );
