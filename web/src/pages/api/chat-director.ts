...
export const POST: APIRoute = async ({ request }) => {
  try {
    const { messages, activeAgents = [] } = await request.json();
        ...
        try {
          ...
          if (messages.length === 1) {
            sendEvent('message', {
              ...
              content: '👋 Welcome! I\'m the Agent Director. I\'ll help coordinate the right specialists to handle your request. Let me analyze what you need...',
              avatar: '🤖',
              timestamp: Date.now(),
            });
          }
          ...
          const conversationHistory = messages.slice(-5).map(m =>
            `${m.role === 'user' ? 'User' : m.metadata?.sender || 'Assistant'}: ${m.content}`
          ).join('\n');
...
${Object.entries(AGENT_PROFILES).map(([id, profile]) =>
  `- ${id}: ${profile.name} ${profile.avatar} (expertise: ${profile.expertise.join(', ')})`
).join('\n')}
...
Think step-by-step about who can add value here.`;
          ...
          let detectedAgents = new Set<string>();
          let currentText = '';
...
Respond now with your contribution:`;
              ...
              try {
                ...
                const agentProvider = createClaudeCode({
                  allowedTools: ['*'], // Give agents full tool access
                });
