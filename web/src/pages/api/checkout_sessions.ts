...
// Dynamic product lookup from content collection
async function getProductById(id: string) {
	try {
		// Try to get product from content collection
		const productEntry = await getEntry("products", id);

		if (!productEntry) {
			// Fallback: check hardcoded products for backwards compatibility
			...
			return hardcodedProducts[id] || null;
		}

		const product = productEntry.data;

		// Convert content collection product to checkout format
		return {
			id: productEntry.slug,
			name: product.name || "Untitled Product",
			price: Math.round((product.price || 0) * 100), // Convert dollars to cents
			description: product.description || "",
			image:
				product.images && product.images.length > 0 ? product.images[0] : "",
			inStock: product.inStock ?? true,
			stock:
				product.variants?.reduce((sum, v) => sum + (v.inStock ? 1 : 0), 0) ||
				10,
		};
	} catch (error) {
...
function validateApiKey(authHeader: string | null): boolean {
	...
	const expectedKey =
		import.meta.env.COMMERCE_API_KEY || "test_key_change_in_production";
...
export const POST: APIRoute = async ({ request }) => {
	try {
		...
		const lineItemsResult = await buildLineItems(items);
		...
		let tax = 0;
		let status: "not_ready_for_payment" | "ready_for_payment" =
