 ...
 */
async function sendOrderWebhookToOpenAI(
	sessionId: string,
	order: any,
): Promise<void> {
...
export const POST: APIRoute = async ({ params, request }) => {
	try {
		...
		if (existingSession.status !== "ready_for_payment") {
			return new Response(
				JSON.stringify({
					...
					code: "invalid",
					message:
						"Session is not ready for payment. Fulfillment address may be missing.",
				} as ErrorResponse),
				{ status: 400, headers: { "Content-Type": "application/json" } },
			);
		}
		...
		// Get total amount
		const totalObj = existingSession.totals.find(
			(t: Total) => t.type === "total",
		);
		if (!totalObj) {
			return new Response(
				JSON.stringify({
		...
		try {
			...
			if (stripeError instanceof Stripe.errors.StripeError) {
				let errorCode: "payment_declined" | "requires_3ds" | "invalid" =
					"payment_declined";
