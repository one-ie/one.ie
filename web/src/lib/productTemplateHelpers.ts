...
export function getProductFeatures(
	...
	const features =
		categoryFeatures[product.category] || categoryFeatures["default"];
	const images =
...
export function getProductSpecs(
	...
	const specs: Array<{ label: string; value: string }> = [
		...
		{ label: "Category", value: categoryCapitalized },
		{ label: "Stock Status", value: `${product.stock} units available` },
		{ label: "SKU", value: `${category.toUpperCase()}-${product.id}` },
	];
 ...
 */
export function getCTAText(category: string): {
	title: string;
	subtitle: string;
} {
	const ctaMap: Record<string, { title: string; subtitle: string }> = {
		fragrances: {
			...
			subtitle:
				"Discover the allure of luxury fragrance. A scent for those who know their power.",
		},
		smartphones: {
			title: "Ready to upgrade?",
			subtitle:
				"Experience the future of mobile technology. Power and elegance in your pocket.",
		},
		"mens-shirts": {
			title: "Ready to elevate your style?",
			subtitle:
				"Step up your wardrobe with timeless elegance. Quality that makes a statement.",
		},
		"womens-dresses": {
			title: "Ready to turn heads?",
			subtitle:
				"Feel confident and beautiful. A dress that celebrates your unique style.",
		},
		default: {
			title: "Ready to own it?",
			subtitle:
				"Experience quality that exceeds expectations. Make it yours today.",
		},
	};
