...
const ERROR_PATTERNS: ErrorPattern[] = [
	...
	{
		matchers: (msg) =>
			msg.includes("already exists") || msg.includes("email is already in use"),
	...
	},
	{
		matchers: (msg) =>
			...
			(msg.includes("token") || msg.includes("link")),
		...
		getCurrentUser: () =>
			Effect.tryPromise({
				try: async () => {
					...
					const user = await client.query("auth:getCurrentUser" as any, {
						token,
					});
