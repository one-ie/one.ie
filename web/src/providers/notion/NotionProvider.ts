...
export const makeNotionProvider = (
	...
	const extractPropertiesFromNotion = async (
		...
		for (const [key, value] of Object.entries(properties)) {
			...
			switch (prop.type) {
				...
				case "relation":
					if (prop.relation) {
						extracted[camelCase(key)] = prop.relation.map((r) =>
							notionIdToOneId(r.id),
						);
					}
					break;
			}
	...
	const transformThingPropertiesToNotion = (
	...
	const mapRelationshipToNotionProperty = (
	...
	return {
		...
		groups: {
			...
			getBySlug: (slug: string) =>
				Effect.fail(
					new GroupNotFoundError(
						slug,
						"Notion provider does not support groups",
					),
				),
			...
			create: () =>
				Effect.fail(
					new GroupCreateError(
						"Notion provider does not support creating groups",
					),
				),
			...
			update: () =>
				Effect.fail(
					new GroupNotFoundError(
						"",
						"Notion provider does not support updating groups",
					),
				),
			...
			delete: () =>
				Effect.fail(
					new GroupNotFoundError(
						"",
						"Notion provider does not support deleting groups",
					),
				),
		},
					...
					try: async () => {
						if (!options?.type) {
							throw new QueryError(
								"Notion provider requires type for listing things",
							);
						}
						...
						if (!databaseId) {
							throw new QueryError(
								`No Notion database configured for type: ${options.type}`,
							);
						}
						...
						new QueryError("Failed to list Notion pages", error),
				}),
					...
					try: async () => {
						...
						if (!databaseId) {
							throw new ThingCreateError(
								`No Notion database configured for type: ${input.type}`,
							);
						}
				...
				Effect.tryPromise({
					try: async () => {
						...
						if (input.properties) {
							Object.assign(
								properties,
								transformPropertiesToNotionUpdate(input.properties),
							);
						}
			...
			list: (options?: ListConnectionsOptions) =>
				Effect.tryPromise({
					try: async () => {
						...
						for (const [propName, propValue] of Object.entries(props)) {
							...
							if (prop.type === "relation" && prop.relation) {
								const relationshipType =
									mapNotionPropertyToRelationship(propName);

								for (const rel of prop.relation) {
									connections.push({
										_id: generateConnectionId(
											fromId,
											notionIdToOneId(rel.id),
											relationshipType,
										),
										fromEntityId: fromId,
						...
						new QueryError("Failed to list Notion connections", error),
				}),
			...
			create: (input: CreateConnectionInput) =>
				Effect.tryPromise({
					try: async () => {
						...
						const propertyName = mapRelationshipToNotionProperty(
			...
			delete: (id: string) =>
				Effect.tryPromise({
					try: async () => {
						...
						// Determine which property to update
						const propertyName =
							mapRelationshipToNotionProperty(relationshipType);
		...
		events: {
			...
			list: (options?: ListEventsOptions) => Effect.succeed([]), // Could integrate with Convex for hybrid storage
		...
		knowledge: {
			...
			list: (options?: SearchKnowledgeOptions) => Effect.succeed([]), // Could integrate with Convex for hybrid storage
