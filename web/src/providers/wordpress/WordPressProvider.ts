...
export const makeWordPressProvider = (
	...
	if (config.auth) {
		const credentials = btoa(
			`${config.auth.username}:${config.auth.applicationPassword}`,
		);
	...
	return {
			...
			get: (id: string) =>
				Effect.fail(
					new ConnectionNotFoundError(
						id,
						"WordPress provider does not support connections",
					),
				),

			list: (options?: ListConnectionsOptions) => Effect.succeed([]), // WordPress doesn't have native connections

			create: (input: CreateConnectionInput) =>
				Effect.fail(
					new ConnectionCreateError(
						"WordPress provider does not support creating connections",
					),
				),
		...
		events: {
			get: (id: string) =>
				Effect.fail(
					new QueryError("WordPress provider does not support events"),
				),

			list: (options?: ListEventsOptions) => Effect.succeed([]), // Could map comments to events in the future

			create: (input: CreateEventInput) =>
				Effect.fail(
					new EventCreateError(
						"WordPress provider does not support creating events",
					),
				),
		},
		...
		knowledge: {
			...
			list: (options?: SearchKnowledgeOptions) =>
				Effect.tryPromise({
					try: async () => {
						...
						new QueryError("Failed to list WordPress categories", error),
				}),

			create: (input: CreateKnowledgeInput) =>
				Effect.fail(
					new QueryError(
						"WordPress provider does not support creating knowledge",
					),
				),
			...
			link: (thingId: string, knowledgeId: string, role?: any) =>
				Effect.fail(
					new QueryError(
						"WordPress provider does not support linking knowledge",
					),
				),
			...
			search: (embedding: number[], options?: SearchKnowledgeOptions) =>
				Effect.fail(
					new QueryError(
						"WordPress provider does not support embedding search",
					),
				),
		},
		...
		auth: {
			login: (args) =>
				Effect.fail(
					new QueryError("WordPress provider does not provide auth") as any,
				),
			signup: (args) =>
				Effect.fail(
					new QueryError("WordPress provider does not provide auth") as any,
...
export const wordPressProvider = (
	baseUrl: string,
	auth?: WordPressProviderConfig["auth"],
