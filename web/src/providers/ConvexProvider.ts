 ...
 * ```
 */
export function createConvexProvider(
		...
		groups: {
			...
			list: (options?: ListGroupsOptions) =>
				toEffect(
					() =>
						client
							.query("groups:list" as any, options || {})
							.then((result) => result as Group[]),
		...
		things: {
			...
			list: (options?: ListThingsOptions) =>
				toEffect(
					() =>
						client
							.query("entities:list" as any, options || {})
							.then((result) => result as Thing[]),
					(message, cause) => new QueryError(message, cause),
				),
		...
		events: {
			...
			list: (options?: ListEventsOptions) =>
				toEffect(
					() =>
						client
							.query("events:list" as any, options || {})
							.then((result) => result as Event[]),
					(message, cause) => new QueryError(message, cause),
				),
